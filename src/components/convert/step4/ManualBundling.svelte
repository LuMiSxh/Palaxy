...
<div class="grid h-full grid-cols-[1fr_1.5fr] gap-6 p-4">
	...
	<div class="flex flex-col gap-4">
		<div class="card">
			<div class="card-body">
				<h3 class="mb-4 text-xl font-semibold">{$t`Bundle Summary`}</h3>

				<div class="space-y-4">
					<div>
						<div class="mb-2 flex items-center justify-between">
							<span class="text-sm font-medium">{$t`Detected Chapters`}</span>
							<span class="badge badge-primary">{step4State.result?.total_chapters ?? 0}</span>
						</div>

						<div class="mb-2 flex items-center justify-between">
							<span class="text-sm font-medium">{$t`Used Chapters`}</span>
							<span class="badge"
										class:badge-success={getTotalChapters() === (step4State.result?.total_chapters ?? 0)}
					...
					<div>
						<div class="mb-2 flex items-center justify-between">
							<span class="text-sm font-medium">{$t`Current Volumes`}</span>
		...
		{#if step4State.result && step4State.result.total_chapters > 0}
			...
			<div class="card">
				<div class="card-body">
					<h3 class="mb-4 text-xl font-semibold">{$t`Chapter Distribution`}</h3>
					<div class="flex flex-wrap gap-1">
						...
						{#each Array(step4State.result.total_chapters) as _, i}
							...
							{@const isExceeded = getTotalChapters() > step4State.result.total_chapters && i >= step4State.result.total_chapters}
	...
	<div class="card h-full overflow-hidden flex flex-col">
		<div class="card-body flex flex-col h-full">
			<h3 class="mb-4 text-xl font-semibold">{$t`Volume Management`}</h3>

			<!-- Volume list -->
			<div
				class="flex-grow overflow-y-auto mb-4 !px-1 !py-1 focus:outline-none rounded focus:ring-2 focus:ring-primary"
				tabindex="0"
				role="tablist"
				bind:this={volumeListContainer}
				onkeydown={handleVolumeListKeyDown}
				onmousedown={preventFocusOnClick}
				use:handleKeyHint={{keys: [['arrowup', $t`Navigate up`], ['arrowdown', $t`Navigate down`], ['delete', $t`Remove selected`]]}}
			>
				{#if convState.chapterSizes.length > 0}
					<div class="space-y-3">
							...
							>
								...
								<div class="flex-1">
									<div class="flex items-center">
										...
										<span class="text-sm">{chapters} {chapters === 1 ? $t`chapter` : $t`chapters`}</span>
									</div>

									<div
										class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-background-tertiary dark:bg-background-dark-tertiary">
								...
								<div class="flex items-center gap-2">
									...
									{:else}
										<!-- Display mode -->
										<button
											class="input h-9 w-16 px-2 text-center flex items-center justify-center"
											onclick={(e) => {
												e.stopPropagation();
												volVisSelectedIdx = i;
												startEditing();
				...
				{:else}
					<div
						...
						<p
							class="text-content-secondary dark:text-content-dark-secondary">{$t`No volumes added yet. Add your first volume below.`}</p>
					</div>
				{/if}
			</div>
			...
			<div class="mt-auto">
				<div class="flex-1">
					<label for="new-volume" class="mb-2 block text-sm font-medium">{$t`Add new volume`}</label>
