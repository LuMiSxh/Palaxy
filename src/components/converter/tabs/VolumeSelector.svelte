<script lang="ts">
	...
	async function runBundler() {
		...
		const result = await bridge<TabVolumeResult>(
			'flow_volume',
			{ basePath: $sourceManga, bundlerFlag: $bundler, sensibility }
		);
		loading = false;
...
<div class="w-full min-h-full flex items-center justify-center flex-col">
	...
	{:else}
		{#if $bundler === BundlerFlag.MANUAL}
			<div
				class="relative table-container max-h-[40vh] w-2/3 overflow-y-auto"
			>
						...
						<tr class="select-none">
							...
							<td class="flex items-center justify-evenly">
								...
								<button
									...
									on:click={() => {$cpv.splice(i, 1); $cpv = $cpv;}}
								>
									<Minus size={18} />
								</button>
							</td>
		...
		{:else}
			<div class="flex w-full h-full items-start justify-evenly">
				<div
					class="table-container w-1/3 max-h-[45vh]"
				>
				...
				{#if $bundler === BundlerFlag.IMAGE}
					<div
						class="w-1/3 flex flex-col items-center justify-center pl-4"
					>
						<RangeSlider
							name="range-slider"
							class="w-full"
